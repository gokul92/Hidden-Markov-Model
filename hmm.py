...
fn = "C:/Users/Gokul/Desktop/NIFTY/Intraday/india/NIFTY 50.csv"
...
# Emmission probability matrix B
...
#********************************EXPECTATION STEP*****************************#
    ...
    for i in range(1, n_data):
        for j in range(n_states):
            ...
            emmis_prob = B[loc, j]
            delta[i,j] = emmis_prob*np.sum(alpha_sc[i-1,:]*A[:,j])
        cn[i] = np.sum(delta[i,:])
    ...
    for i in range(n_data-2, -1, -1):
        for j in range(n_states):
            ...
            emmis_prob = B[loc, j]
            epsilon[i,j] = np.sum(beta_sc[i+1,:]*emmis_prob*A[j,:])
    ...
    for i in range(n_states):
        for j in range(n_states):
            ...
            for n in range(0, n_data-1):
                ...
                emmis_prob = B[loc,i]
                numerator = numerator + alpha_sc[n-1,i]*beta_sc[n,j]*emmis_prob*A[i,j]/cn[n]
            for l in range(n_states): 
                temp = 0
                for n in range(0, n_data-1):
                    loc = np.where(x[n] == x_states)[0]
                    emmis_prob = B[loc,l]
                    temp = temp + alpha_sc[n-1,i]*beta_sc[n,l]*emmis_prob*A[i,l]/cn[n]
